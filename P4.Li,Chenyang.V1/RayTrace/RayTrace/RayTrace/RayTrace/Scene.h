...
public:
	...
	//-intersection-
	float intersection(Vector origin, Vector direction)
	{
		float t;
		float t1, t2;
		float deltasquare;
		...
		b = direction.Dot((origin - center)) * 2.0;
		c = (origin - center).Dot((origin - center)) - powf(radius, 2.0);
		deltasquare = powf(b, 2.0) - 4.0 *a* c;
		...
		if (deltasquare > 0)
		{
			...
			t1 = (-b - sqrtf(deltasquare)) / (2.0 * a);
			t2 = (-b + sqrtf(deltasquare)) / (2.0 * a);
